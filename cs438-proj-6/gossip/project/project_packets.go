...
type EncFileInfo struct {
	...
	Points    *Points `json:"points"`    //encryption of the AES Key
	Iv        []byte  `json:"iv"`        //iv for decryption of encInfo
	EntryHash string  `json:"entryhash"` //hex of Sha1(OwnerID||Filename)
	EncInfo   []byte  `json:"encinfo"`   //encrypted info with AES
}

//This is the encryption of an AES key over elliptic curves
type Points struct {
...
type PartialDecrypt struct {
	...
	C         []byte `json:"C"`         //C of encrypted AES key
	Iv        []byte `json:"iv"`        //iv for decrypting encInfo
	EncInfo   []byte `json:"encinfo"`   //encrypted information
}
